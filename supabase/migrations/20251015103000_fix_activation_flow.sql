-- Align activation expiry checks with the stored key and type
-- and add wrapper RPCs matching frontend expectations
...
DECLARE
  ...
  v_auth_user_id uuid;
BEGIN
  -- Validate token against epoch-based expiry key
  ...
  WHERE u.profile_data->>'activation_token' = p_activation_token
    ...
    AND to_timestamp((u.profile_data->>'activation_token_expires')::double precision) > now();
  ...
  SET 
    ...
    email_verified = true,
    profile_data = profile_data - 'activation_token' - 'activation_token_expires'
...
-- Wrapper functions matching frontend RPC names
CREATE OR REPLACE FUNCTION public.user_activate_account(p_activation_token text)
RETURNS jsonb
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT public.activate_user_account(p_activation_token);
$$;

CREATE OR REPLACE FUNCTION public.user_activate_account_with_code(
  p_activation_token text,
  p_password text
)
RETURNS jsonb
...
SET search_path = public
AS $$
  SELECT public.activate_user_account_with_password(p_activation_token, p_password);
$$;
