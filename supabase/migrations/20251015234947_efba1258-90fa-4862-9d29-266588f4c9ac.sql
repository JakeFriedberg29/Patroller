-- Fix user_create_with_activation to use gen_random_uuid instead of gen_random_bytes
...
BEGIN
  ...
  -- Generate a temporary password using gen_random_uuid (always available)
  v_temp_password := replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', '');
